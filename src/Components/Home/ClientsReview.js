...
const ClientsReview = () => {
    ...
    const { data: reviews, isLoading, refetch } = useQuery('reviewDataHome', () => fetch(`${rootUrl}/getReviewLimit/3`).then(res => res.json()));
                    ...
                    reviews?.map(({ rating, text, userEmail, userImg, userName }, i) =>
                        <div key={i} className="stack ">
                            <div className="card glass border-[1px] border-white  card-side bg-base-100 shadow-xl">
                                <figure className='' ><img className='w-36 rounded-full  ring ring-slate-400 ring-offset-base-100 ring-offset-2 m-3 ' src={userImg} alt="Movie" /></figure>
                                <div className="card-body p-5 ">
                                    ...
                                    <div className="card-actions justify-end">
                                        <div class="card-actions justify-end">
                                            <div class="badge badge-outline">{userEmail}</div>
                                            <div class="badge badge-outline text-black bg-green-500 ">{userName}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
